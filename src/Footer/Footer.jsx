      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-md ">
            <p className="dark:text-slate-100 text-2xl font-semibold text-slate-900">Medicare</p>
              ...
              <ul className="space-y-2 text-sm text-slate-600">
                {navLinks.slice(0, 4).map((link) => (
                  <li key={link.to}>
                    <NavLink
                      to={link.to}
                      className="transition hover:text-slate-900 dark:text-slate-100"
                    >
                      {link.label}
                    </NavLink>
                  </li>
        ...
        <div className="mt-10 dark:border-gray-600 border-t border-slate-200 pt-6 text-sm text-slate-500 sm:flex sm:items-center sm:justify-between">
          <p className="dark:text-slate-100">© 2026 Medicare. All rights reserved.</p>
          <div className="mt-4 flex flex-wrap gap-4 sm:mt-0">
            {navLinks.map((link) => (
              <NavLink
                key={link.to}
                to={link.to}
                className="text-slate-600 transition hover:text-slate-900 dark:text-slate-100"
              >
                {link.label}
              </NavLink>
            ))}
